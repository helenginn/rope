...
int main()
{
	...
	Atom e, f, g, h;
	e.setInitialPosition(glm::vec3(2, +1, 30));
	f.setInitialPosition(glm::vec3(2,  0, 30));
	g.setInitialPosition(glm::vec3(4,  0, 30));
	h.setInitialPosition(glm::vec3(4, +1, 30));
	BondTorsion *identical = new BondTorsion(nullptr, &e, &f, &g, &h, 2);

	double similar = torsion->similarityScore(identical);

	return !(fabs(similar - 0) < 1e-6);
}
