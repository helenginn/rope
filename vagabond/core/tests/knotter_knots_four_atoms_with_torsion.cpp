...
int main()
{
	...
	if (group.bondTorsionCount() != 1)
	{
		std::cout << "Incorrect torsion angle count" << std::endl;
		return 1;
	}
	
	if (k.warningCount() > 0)
	{
		std::cout << "Warning count is " << k.warningCount() << std::endl;
		
		for (size_t i = 0; i < k.warningCount(); i++)
		{
			std::cout << k.warning(i) << std::endl;
		}
