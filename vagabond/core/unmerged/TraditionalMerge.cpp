...
TraditionalMerge::TraditionalMerge()
{

}


void TraditionalMerge::operator+=(StillImage &image)
{
	for (const Obs &obs : image)
	{
		_info[obs.hkl].push_back(obs.info);
	}
}

void TraditionalMerge::operator()(const std::string &filename)
{
	...
	data.reserve(_info.size() * 4);
	for (auto it = _info.begin(); it != _info.end(); it++)
	{
		std::vector<Info> &group = it->second;
		...
		for (Info &info : group)
		{
			merged += info.intensity * info.weight;
			weights += info.weight;
		}
