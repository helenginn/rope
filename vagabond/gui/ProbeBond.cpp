...
void ProbeBond::updateProbe()
{
	...
	_batch->setSlotImage(_slot, result);
...
void ProbeBond::updatePosition()
{
	...
	_batch->fixVertices(_slot, start + truncate * left,
	                    end - start + truncate * right);

	_batch->forceRender(true, true);
}
...
ProbeBond::ProbeBond(ProtonNetworkView *view, ProbeBondBatch *batch,
                    BondProbe *probe)
{
	_batch = batch;
	_view = view;
	_probe = probe;
	_slot = _batch->appendBond(this);
...
void ProbeBond::selected(int idx, bool inverse)
{
	...
	_batch->setSlotColour(_slot, colour, colour, colour * 2.f);
